...
def likes():
    # return '''<h1>Ваша корзина</h1>'''
    db_sess = db_session.create_session()
    # l_c = db_sess.query(Likes).filter(Products.id_products == id_products, Products.user_id == user_id).first()
    products = list(db_sess.query(likes))
    # print(products)
    # for i in range(len(products)):
    #    print(products[i].user.name)
    #products = [products[i:i + 2] for i in range(0, len(products), 2)]
...
@app.route('/likes/<int:id>', methods=['GET', 'POST'])
def add_likes():
    pass
...
@app.route('/shop_cart/<int:id>', methods=['GET', 'POST'])
